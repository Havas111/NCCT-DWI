...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        self.lpips_fn=lpips.LPIPS(net='alex', version='0.1').to(torch.device('cuda:1'))
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        ...
        if self.steps[i] == 0:
            ...
            model_output = self.denoise_fn(x_t, timesteps=t, context=context)
            if isinstance(model_output, tuple):
                objective_recon, _ = model_output
            else:
                objective_recon = model_output
            ...
            x_tminus_mean = (1. - m_nt) * x0_recon + m_nt * y_content + torch.sqrt((var_nt - sigma2_t) / var_t) * \
                            (x_t - (1. - m_t) * x0_recon - m_t * y_content)
    ...
    def p_sample_loop(self, y, context=None, clip_denoised=True, sample_mid_step=False):
        ...
        y_content = y[:, 0:1, :, :]
        if sample_mid_step:
            imgs, one_step_imgs = [y_content], []
            for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
                img, x0_recon = self.p_sample(x_t=imgs[-1], y=y_content, context=context,  i=i, clip_denoised=clip_denoised)
                imgs.append(img)
            ...
            img = y_content
            for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
                img, _ = self.p_sample(x_t=img, y=y_content, context=context, i=i, clip_denoised=clip_denoised)
            return img
