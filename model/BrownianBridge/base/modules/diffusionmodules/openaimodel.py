...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # self.context_proj = nn.Linear(1, 512)

        self.anatomy_encoder = AnatomyEncoder(input_dim=2, output_dim=1)
        self.spatial_refiner = nn.Sequential(
            conv_nd(dims, 3, 16, 3, padding=1),
            nn.SiLU(),
            zero_module(conv_nd(dims, 16, 1, 3, padding=1))
        )
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if self.condition_key != 'nocond':
            ct_original = context[:, 0:1, :, :]
            pos_original = context[:, 1:2, :, :]
            learned_mask = self.anatomy_encoder(context)
            refined_pos = pos_original * learned_mask
            context = torch.cat([ct_original, refined_pos], dim=1)
