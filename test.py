...
sys.modules["wandb.apis"] = MagicMock()
...
if np.__version__.startswith("2."):
    ...
    for old_attr, new_type in mapping.items():
        if not hasattr(np, old_attr):
            setattr(np, old_attr, new_type)
import torch
import random
import numpy as np
...
def parse_args_and_config():
    ...
    parser.add_argument('-r', '--result_path', type=str, default=f'results/{exp_name}', help="The directory to save results")
    ...
    parser.add_argument('--gpu_ids', type=str, default=None, help='gpu ids, 0,1,2,3 cpu=-1')
